...
This guide walks you through setting up **Community Hub** on Windows. By the end, you will have the full stack running locally and be ready to contribute.
...
> **Strongly recommended:** Use [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) (Ubuntu) for Path B. Run all backend and frontend commands inside your WSL2 terminal. Docker Desktop integrates with WSL2 automatically.

### Git

Download and install [Git for Windows](https://git-scm.com/download/win), or install Git inside WSL2:
...
2. Enable **WSL2 backend** in Docker Desktop settings (Settings → General → *Use the WSL 2 based engine*).
3. Under Settings → Resources → WSL Integration, enable integration for your Ubuntu distro.
...
Start Docker Desktop before running any `docker compose` commands.
...
**Native Windows (alternative):** Install from [nodejs.org](https://nodejs.org/) or use [nvm-windows](https://github.com/coreybutler/nvm-windows).
...
**Native Windows (alternative):** Download from [python.org](https://www.python.org/downloads/) or use [pyenv-win](https://github.com/pyenv-win/pyenv-win).
...
> For Path B, run commands in a **WSL2 Ubuntu terminal** unless noted otherwise. `bash scripts/dev.sh` requires WSL2 or Git Bash — it does not run in plain PowerShell.
...
> Run these steps inside **WSL2 Ubuntu** for the smoothest experience.
...
Or use the convenience script (WSL2 or Git Bash):
...
| Docker commands fail | Docker Desktop not running or WSL integration off | Start Docker Desktop; enable WSL integration in settings |
| `bash scripts/dev.sh` fails in PowerShell | Script requires Bash | Use WSL2 or Git Bash instead |
| Line ending errors in shell scripts | Windows CRLF line endings | Run `dos2unix scripts/dev.sh` in WSL2, or clone with `git config --global core.autocrlf input` |
