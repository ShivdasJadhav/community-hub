...
This guide walks you through setting up **Community Hub** on macOS. By the end, you will have the full stack running locally and be ready to contribute.
...
```bash
xcode-select --install   # includes Git, if not already installed
git --version
```
...
Start Docker Desktop before running any `docker compose` commands. Works on both Apple Silicon and Intel Macs with no extra configuration.
...
No Node.js, Python, or uv required on your Mac.
...
Run Postgres and Redis in Docker; run the backend and frontend natively on your Mac for faster iteration and easier debugging.
...
| Docker commands fail | Docker Desktop not running | Open Docker Desktop and wait until it is ready |
